...
const Custom = () => {
  const [displayCustomSaction, setDisplayCustomSaction] = useState(false);
  const [Saction, setSaction] = useState("");
  const [currentIndex, setcurrentIndex] = useState(0);
  const [CustomSaction, setCustomSaction] = useState([
    {
      title: "",
      Section: " ",
      isShow: true,
  ...
  const onDropDwon = (id) => {
    setCustomSaction(
      CustomSaction.map((x) => (x.id == id ? { ...x, isShow: !x.isShow } : x))
    );
  };

  const handleCustomSactionChange = (index, event) => {
    setcurrentIndex(index);
    console.log(event.target.value);
    let data = [...CustomSaction];
    data[index][event.target.name] = event.target.value;
    setCustomSaction(data);
  };

  useEffect(() => {
    let data = [...CustomSaction];
    if (Saction) data[currentIndex].Saction = Saction;

    Saction && setCustomSaction(data);
  }, [Saction]);

  const addFieldsCustomSaction = () => {
    setDisplayCustomSaction(true);
    if (!displayCustomSaction) {
      ...
      let newData = {
        ...
        Section: " ",
        isShow: true,
        id: CustomSaction.length,
      };

      const oldData = CustomSaction.map((x) => {
        return {
          ...x,
          isShow: false,
        };
      });
      setCustomSaction([...oldData, newData]);
    }
  };

  const removeFieldsCustomSaction = (index, e) => {
    e.preventDefault();
    e.stopPropagation();
    e.nativeEvent.stopImmediatePropagation();
    let data = [...CustomSaction];
      ...
      {({ values }) => (
        <div className="mt-5 p-5 bg-white rounded-md shadow-md">
          <label for="large-input" className="flex justify-between item-center">
            ...
            <span onClick={() => setDisplayCustomSaction((Prev) => !Prev)}>
              {displayCustomSaction ? (
               <svg 
               className="w-9 h-9"
               viewBox="0 0 52 51" 
               fill="none" 
          ...
          <div className={!displayCustomSaction ? "hidden" : "block"}>
            {CustomSaction.map((input, index) => (
              <Form
                onSubmit={submit}
              ...
              >
                ...
                >
                  {input.title ? input.title : "(Not Specified)"}
                  ...
                  <div key={index} className="mb-3 ">
                    <div className="grid gap-6 mb-6 laptop:grid-cols-2">
                      <div>
                        ...
                        <Field
                          ...
                          value={input.title}
                          onChange={(event) =>
                            handleCustomSactionChange(index, event)
                          }
                        ></Field>
                        <ErrorMessage name="tit,e" className='text-red-500 text-xs italic'/>
                      </div>
                    </div>
                    <div className="w-full">
                      <label
                        ...
                        //   className="block mb-2 text-sm font-medium dark:text-black"
                      >
                        Saction <span className="text-red-700">*</span>
                      </label>
                      <Field
                        as="textarea"
                        ...
                        placeholder=""
                      />
                      <ErrorMessage name="Saction" className='text-red-500 text-xs italic'/>
                    </div>
                  </div>
                </div>

                <button
                  onClick={(e) => removeFieldsCustomSaction(index, e)}
          ...
          {CustomSaction.length >= 0 && displayCustomSaction && (
            <div
              onClick={addFieldsCustomSaction}
              className="m-2 w-full cursor-pointer text-blue-900 hover:text-blue-500 font-bold text-left"
            >
              {CustomSaction.length == 0
                ? "+ Add Custom saction"
                : "+ Add more Custom saction"}
            </div>
          )}
        </div>
      )}
